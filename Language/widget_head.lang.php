<?php
/**
 * Created by PhpStorm.
 * User: Demort
 * Date: 24.09.2019
 * Time: 10:46
 */

return array(
    'ru' => array(

        'show_info' => array(
            'Возникла проблема? Обратись в Саппорт!',
            'Хочешь больше информации? Зайди на форум!',
            'Ты должен знать об акциях, которые сейчас проходят!',
            'Подпишись на рассылку,будь в курсе!',
            'Наверняка что-то произошло в мире, обсуди это на форуме!',
            'Скучно? Раздел портовый кабак на форуме скрасит время.',
            'Что-то не понятно в игре? Тогда тебе на Форум!',
            'Хочешь высказаться? Не держи в себе! Зайди на форум!',
        ),


        'lang_account_ban' => 'Ваш аккаунт заблокирован',
        'lang_title_master_account' => 'Мастер аккаунт',
        'lang_btn_go_settings' => 'Перейти к настройкам',
        'lang_btn_go_check_email' => 'Потвердить почту',
        'lang_btn_go_check_phone' => 'Привязать телефона',
        'lang_btn_last_enter' => 'Последний вход',
        'lang_btn_reset_pin' => 'Сбросить PIN-code',


        'lang_title_warehouse' => 'Склад',
        'lang_btn_go_warehouse' => 'Открыть склад',
        'lang_btn_go_info_warehouse' => 'Подробнее',


        'lang_title_balance' => 'Баланс',
        'lang_btn_go_transfer_game' => 'Перевести в игру',
        'lang_btn_go_enroll' => 'Пополнить',
        'lang_btn_go_refresh' => 'Обновить',


        'lang_title_sale' => 'Скидка',
        'lang_btn_go_info_sale' => 'Подробно о скидке',




    ),
    'en' => array(

        'show_info' => array(
            'Having a problem? Contact Support!',
            'Want more info? Go to the forum!',
            'You should know about the promotions that are taking place now!',
            'Sign up for our newsletter!',
            'Surely something happened in the world, discuss it on the forum!',
            'Bored? Section port tavern forum will brighten up the time.',
            'Something is not clear in the game? Then read the forum!',
            'Do you want to speak out? Do not keep in yourself! Go to the forum!',
        ),


        'lang_account_ban' => 'Your account has been suspended.',
        'lang_title_master_account' => 'Master account',
        'lang_btn_go_settings' => 'Go to settings',
        'lang_btn_go_check_email' => 'Confirm Mail',
        'lang_btn_go_check_phone' => 'Confirm phone',
        'lang_btn_last_enter' => 'Last entrance',
        'lang_btn_reset_pin' => 'Reset PIN',


        'lang_title_warehouse' => 'Warehouse',
        'lang_btn_go_warehouse' => 'Open warehouse',
        'lang_btn_go_info_warehouse' => 'More details',


        'lang_title_balance' => 'Balance',
        'lang_btn_go_transfer_game' => 'Transfer to the game',
        'lang_btn_go_enroll' => 'Top up balance',
        'lang_btn_go_refresh' => 'Update',


        'lang_title_sale' => 'Discount',
        'lang_btn_go_info_sale' => 'Discount Details',
	
	
	
    ),
);